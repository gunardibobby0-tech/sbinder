...
Create a production-ready script with proper industry formatting. 
Use [SCENE HEADING], [ACTION], [CHARACTER], and [DIALOGUE] tags clearly.
${langInstruction}`;
  } else {
    systemPrompt = `You are an award-winning screenwriter. 
Deliver a technically perfect script following standard screenplay format:
- Scene Headings (EXT. LOCATION - DAY)
- Action lines (Present tense, visual)
- Character names (Centered/Uppercase)
- Dialogue with parentheticals where needed
Ensure the narrative has a clear beginning, middle, and end with strong character beats.
${langInstruction}`;
  }
